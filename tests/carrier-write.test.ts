...
before(async () => {
  ...
  userA = db.get<{ id: number }>("SELECT id FROM users WHERE email = 'a@x.test'")!.id;
  userB = db.get<{ id: number }>("SELECT id FROM users WHERE email = 'b@x.test'")!.id;

  const look = (kind: string, value: string) =>
    db.get<{ id: number }>("SELECT id FROM lookups WHERE kind = ? AND value = ?", [kind, value])!.id;
  ids = {
...
test("a patch never blanks fields it does not mention", () => {
  ...
  const before_ = carriers.getCarrier(id)!;

  write.updateCarrier(id, { status_id: ids.active }, userB);

  const after_ = carriers.getCarrier(id)!;
...
test("an explicit null clears a field, unlike an absent key", () => {
  ...
  write.updateCarrier(id, { owner_name: null }, userA);
  assert.equal(carriers.getCarrier(id)!.owner_name, null);
});

test("a status change is recorded with both old and new labels", () => {
  const id = makeCarrier({ mc_number: "444444", usdot: "2444444" });
  write.updateCarrier(id, { status_id: ids.suspended }, userB);

  const act = db.get<{ type: string; summary: string; old_value: string; new_value: string; user_id: number }>(
    "SELECT type, summary, old_value, new_value, user_id FROM carrier_activity WHERE carrier_id = ? ORDER BY id DESC",
    [id],
  )!;
...
test("each kind of change is filed under the right activity type", () => {
  ...
  db.run("DELETE FROM carrier_activity WHERE carrier_id = ?", [id]);
  ...
  const byType = new Map(
    db.all<{ type: string; field: string }>(
      "SELECT type, field FROM carrier_activity WHERE carrier_id = ?", [id],
...
test("re-saving identical values writes nothing", () => {
  ...
  assert.equal(
    db.get<{ n: number }>("SELECT COUNT(*) AS n FROM carrier_activity WHERE carrier_id = ?", [id])!.n,
    0,
    "no phantom history entries",
  );
  assert.equal(carriers.getCarrier(id)!.updated_at, before_.updated_at, "updated_at untouched");
});

test("a string from a form equals the number already stored", () => {
  const id = makeCarrier({ mc_number: "888888", usdot: "2888888" });
  const result = write.updateCarrier(id, { status_id: String(ids.upcoming) }, userA);
...
test("duplicate MC and USDOT are found regardless of formatting", () => {
  ...
  const byMc = carriers.findDuplicates("999999", null);
  assert.equal(byMc.mc.length, 1);
  assert.equal(byMc.mc[0]!.legal_name, "Original Hauling LLC");

  assert.equal(carriers.findDuplicates("MC-999999", null).mc.length, 1, "formatting ignored");
  assert.equal(carriers.findDuplicates(null, "2999999").usdot.length, 1);
  assert.equal(carriers.findDuplicates("123123123", null).mc.length, 0);
  assert.equal(carriers.findDuplicates(null, null).mc.length, 0);
});

test("a carrier is not flagged as a duplicate of itself when edited", () => {
  const id = makeCarrier({ mc_number: "121212", usdot: "2121212" });
  assert.equal(carriers.findDuplicates("121212", "2121212", id).mc.length, 0);
  assert.equal(carriers.findDuplicates("121212", "2121212", id).usdot.length, 0);
  assert.equal(carriers.findDuplicates("121212", null).mc.length, 1, "still found for others");
});

test("updating an unknown carrier throws rather than writing", () => {
  assert.throws(() => write.updateCarrier(987654, { status_id: ids.active }, userA), /not found/i);
});
