...
before(async () => {
  ...
  const statusId = db.get<{ id: number }>(
    "SELECT id FROM lookups WHERE organization_id = ? AND kind = 'status' AND value = 'active'",
    [orgId],
  )!.id;
  db.run(
    `INSERT INTO carriers (organization_id, legal_name, status_id, created_at, updated_at)
     VALUES (?, 'Fixture Carrier', ?, ?, ?)`,
    [orgId, statusId, new Date().toISOString(), new Date().toISOString()],
  );
...
test("stores a note with author attribution", () => {
  const result = notes.createNote({ org, carrierId, userId: 1, body: "  Called the owner.  " });
  assert.deepEqual(result, { ok: true });

  const row = db.get<{ body: string; user_id: number; pinned: number }>(
    "SELECT body, user_id, pinned FROM carrier_notes WHERE organization_id = ? AND carrier_id = ? ORDER BY id DESC",
    [org.id, carrierId],
  )!;
...
test("a routine note does not clutter the activity timeline", () => {
  const before_ = db.get<{ n: number }>(
    "SELECT COUNT(*) AS n FROM carrier_activity WHERE organization_id = ? AND carrier_id = ?", [org.id, carrierId],
  )!.n;
  notes.createNote({ org, carrierId, userId: 1, body: "Routine check-in." });
...
test("an important note is pinned and recorded in activity", () => {
  notes.createNote({ org,
    carrierId, userId: 1, important: true, body: "Insurance lapses next month.",
  });

  const note = db.get<{ pinned: number }>(
    "SELECT pinned FROM carrier_notes WHERE organization_id = ? AND carrier_id = ? ORDER BY id DESC", [org.id, carrierId],
  )!;
  assert.equal(note.pinned, 1);

  const activity = db.get<{ type: string; summary: string; user_id: number }>(
    "SELECT type, summary, user_id FROM carrier_activity WHERE organization_id = ? AND carrier_id = ? ORDER BY id DESC",
    [org.id, carrierId],
  )!;
...
test("rejects an empty body and an unknown carrier", () => {
  assert.deepEqual(notes.createNote({ org, carrierId, userId: 1, body: "   " }), {
    ok: false, error: "Write something before saving the note.",
  });
  assert.deepEqual(notes.createNote({ org, carrierId: 999999, userId: 1, body: "hi" }), {
    ok: false, error: "Unknown carrier.",
  });
});

test("long notes are truncated rather than rejected", () => {
  notes.createNote({ org, carrierId, userId: 1, body: "x".repeat(notes.MAX_NOTE + 500) });
  const row = db.get<{ body: string }>(
    "SELECT body FROM carrier_notes WHERE organization_id = ? AND carrier_id = ? ORDER BY id DESC", [org.id, carrierId],
  )!;
...
test("a note cannot be attributed to a user who does not exist", () => {
  assert.throws(
    () => notes.createNote({ org, carrierId, userId: 4242, body: "Ghost author." }),
    /FOREIGN KEY constraint failed/,
  );
});

test("pin toggles both ways and reports the carrier to revalidate", () => {
  notes.createNote({ org, carrierId, userId: 1, body: "Toggle me." });
  const id = db.get<{ id: number }>(
    "SELECT id FROM carrier_notes WHERE organization_id = ? AND carrier_id = ? ORDER BY id DESC", [org.id, carrierId],
  )!.id;

  assert.equal(notes.toggleNotePin(org, id), carrierId);
  assert.equal(db.get<{ pinned: number }>("SELECT pinned FROM carrier_notes WHERE organization_id = ? AND id = ?", [org.id, id])!.pinned, 1);
  assert.equal(notes.toggleNotePin(org, id), carrierId);
  assert.equal(db.get<{ pinned: number }>("SELECT pinned FROM carrier_notes WHERE organization_id = ? AND id = ?", [org.id, id])!.pinned, 0);
  assert.equal(notes.toggleNotePin(org, 999999), null);
});

test("notes are ordered important-first, then newest-first", () => {
  const rows = db.all<{ pinned: number }>(
    `SELECT pinned FROM carrier_notes WHERE organization_id = ? AND carrier_id = ?
     ORDER BY pinned DESC, created_at DESC, id DESC`, [org.id, carrierId],
  );
